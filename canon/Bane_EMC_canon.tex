...
\column{.50\textwidth} % Left column and width
	...
	\begin{itemize}
		\item JLab experiment E03103 \cite{E3103} 
		\item CERN, SLAC, HERMES, BCDMS, and JLab
		\item Quantized by slope of A/D ratios from 0.3 - 0.7 in x
		\item Corrections for non-isoscalar nuclei
...
\end{frame}
\begin{frame}
\frametitle{The JLab Tritium Collaboration}
\vspace{-15pt}
...
\vspace*{-1.15cm}
		\begin{itemize}
			\begin{columns}[t]
				\column{0.45\textwidth}
					\onslide<1->{\item N = Number of electrons}
					\onslide<1->{\vfill \item BG = Background}
					\onslide<1->{\vfill \item L = Luminosity }
					\vspace{-10pt}
					\begin{itemize}
						\onslide<1->{\item Density Correction }
					\end{itemize}
				\column{0.45\textwidth}
					\onslide<1->{\item $\epsilon$ = Efficiency }
					\onslide<1->{\item $\Delta E^{\prime} \Delta \Omega$= Bin Size }
					\onslide<1->{\item $A(E^{\prime},\theta)$ = Acceptance probability }
			\end{columns}
		\end{itemize}
...
\begin{block}{Efficiencies ($\epsilon$)}
	\begin{columns}[t]
		...
		\begin{itemize}
			\onslide<1->\item Use well defined samples from separate system(s)
			
						\onslide<1->{
				\begin{itemize}
				\item Testing cherenkov use calorimeters
				\item Testing VDC use scintillators. 				\end{itemize}						}
			\onslide<1->\item Determine good event samples with cuts
			\onslide<1->\item \# of events which fail the criteria $\rightarrow$ inefficiency 
		\end{itemize}
		...
		\onslide<1->{	\item Particle Identification(PID) 
			\begin{itemize}
				\item Cherenkov
				\item Calorimeters
			\end{itemize}}
		\onslide<1->{	\item Trigger
			\begin{itemize}
				\item Scintillators \& cherenkov
				\item $\approx$ 99\%
			\end{itemize}}
		\onslide<1->{	\item Tracking
			\begin{itemize}
				\item Vertical Drift Chambers(VDCs)
				\item $\approx$ 98\%
			\end{itemize}}
		\onslide<1->{	\item Electronic Lifetime
			\begin{itemize}
				\item $\approx$ 96\%-99\%
			\end{itemize}}
		\end{itemize}
...
\column{0.65\textwidth}
\begin{figure}[t]%
	\includegraphics[width=8cm]{../images/pid_2dn.png}
\end{figure}
\column{0.35\textwidth}
\begin{itemize}
...
\begin{columns}
	...
	\begin{itemize}
		...
		\item Fit use to extrapolate correction
	\end{itemize}
	\column{0.45\textwidth}
	\vspace{-.95cm}
	\begin{figure}
...
\begin{block}{Beta Decay}
\begin{columns}
	\column{0.55\textwidth}
	\vspace*{-.55cm}
\begin{figure}

	\includegraphics[width=5cm]{../images/beta_decay.png}
\end{figure}	
	\column{0.55\textwidth}
	\vspace*{-1.75cm}
	\begin{itemize}
		\item $^3H$ beta decays into $^3He$
		\item Time depended correction
	\end{itemize}
...
\begin{frame}
\vspace*{-.50cm}
\begin{block}{EndCap Subtraction}
	\vspace*{-.250cm}	
		\begin{figure}
			\includegraphics[width=7.0cm]{../images/endcap_kin4.pdf}
		\end{figure}

\end{block}
\vspace*{-.250cm}
\begin{block}{Density Correction}

\begin{columns}
	\column{0.45\textwidth}
%	\vspace*{-.750cm}
	\begin{itemize}
		\item Local density fluctuations due to beam heating
		\item \footnotesize{S.N. Santiesteban, J. Bane, et. al \emph{Nucl. Instrum. Meth. A} 940 (2019) 351-358} 
	\end{itemize}
	\column{0.65\textwidth}
	\vspace*{-1.25cm}
	\begin{figure}
		\includegraphics[height=4.0cm]{../images/T2_dens.png}
		\vspace{-0.5cm}
...
\begin{frame}
	...
	\onslide<1->{Monte Carlo simulation : \\}
	\onslide<1->{$\bullet$ $(\Delta E^{\prime} \Delta \Omega)_{Data}$ =$(\Delta E^{\prime} \Delta \Omega)_{MC}$\\ }
	\onslide<1->{$\bullet$ $A(E^{\prime},\theta)_{Data} = A(E^{\prime},\theta)_{MC}$}
	\onslide<1->{
	\begin{equation}
...
\textbf{Yield}                & 0.01           & 0.0107         & 0.0149        & 0.0151        & 0.0141         & 0.0163         \\ \hline
Stat *                         & 0.0055         & 0.0059         & 0.01          & 0.0111        & 0.0113         & 0.0143         \\ \hline
End Cap*                            & 0.007          & 0.007          & 0.007         & 0.007         & 0.007          & 0.007          \\ \hline
Efficiency *                          & 0.004          & 0.0051         & 0.0083        & 0.0071        & 0.0041         & 0.0032         \\ \hline\hline
...
\textbf{Total}                & 0.019          & 0.018          & 0.02          & 0.022         & 0.033          & 0.04           \\ \hline
\end{tabular}
...
\begin{figure}
	\includegraphics[width =11cm,height=5cm]{../images/Thesis/ISOfacs1.pdf}
\end{figure}
...
\begin{block}{Special Thanks}
	\begin{itemize}
		...
		\item CMRU for allowing me the opportunity to present. 
	\end{itemize}
\end{block}
\end{frame}
...
\begin{frame}{}
\begin{block}{Refereed Papers}
	\begin{itemize}
		
		\item R. Cruz Torres. et. al. Comparing proton momentum distributions in A=2 and 3 nuclei via $^2$H $^3$H and $^3$He  (e,e$^{\prime}$p) measurements. \emph{Phys. Let. B 797}, (2019)
		\item S. N. Santiesteban, et. al. Density Changes in Low Pressure Gas Targets for Electron Scattering Experiments. \emph{Nucl. Instrum. Meth. A 940} (2019) 
		\item H. Dai, et. al. First Measurement of the Ar(e,e$^\prime$) Cross Section at Jefferson Lab. \emph{Phys. Rev. C 99}, (2019)
		\item H. Dai, et. al. First Measurement of the Ti(e,e$^\prime$) Cross Section at Jefferson Lab. \emph{Phys. Rev. C 98}, (2019)
	\end{itemize} 
\end{block}
\end{frame}
